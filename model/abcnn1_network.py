...
class Abcnn1(nn.Module):
    ...
    def __init__(self, embed_size=32, sentence_length=40, filter_width=3, filter_channel=32, layer_size=2, match='cosine',
                 inception=True):
        ...
        self.ap = nn.ModuleList([ApLayer(embed_size)])
        self.wp = nn.ModuleList()
        # self.fc = nn.Linear(layer_size + 1, 2)
        self.fc = nn.Sequential(
            nn.BatchNorm1d(layer_size + 1,),
            nn.Linear(layer_size + 1, layer_size + 1),
            nn.ReLU(inplace=True),
            nn.BatchNorm1d(layer_size + 1),
            nn.Dropout(0.5),
            nn.Linear(layer_size + 1, 2),
        )


        for i in range(layer_size):
            self.abcnn.append(Abcnn1Portion(sentence_length, embed_size if i == 0 else filter_channel))
            self.conv.append(
                ConvLayer(False, sentence_length, filter_width, embed_size if i == 0 else filter_channel, filter_channel,
                          inception))
    ...
    def forward(self, x1, x2):
        ...
        output : 2-D torch Tensor
            size (batch_size, 1)
        '''
        if len(x1.shape)==3:  # match the input tensor shape from 3D to 4D
...
class Abcnn2(nn.Module):
    ...
    def __init__(self, embed_size=32, sentence_length=40, filter_width=3, filter_channel=100, layer_size=2, match='cosine',
                 inception=False):
        ...
        for i in range(layer_size):
            ...
            self.conv.append(
                ConvLayer(True, sentence_length, filter_width, embed_size if i == 0 else filter_channel, filter_channel,
                          inception))
            self.ap.append(ApLayer(filter_channel))
...
class Abcnn3(nn.Module):
    ...
    def __init__(self, embed_size=32, sentence_length=40, filter_width=3, filter_channel=100, layer_size=2, match='cosine',
        ...
        for i in range(layer_size):
            self.abcnn1.append(Abcnn1Portion(sentence_length, embed_size if i == 0 else filter_channel))
...
class InceptionModule(nn.Module):
    ...
    def __init__(self, in_channel, sentence_length, filter_width, filter_height, filter_channel):
        ...
        self.conv_2 = convolution(in_channel, filter_width + 4, filter_height, int(filter_channel / 3),
                                  filter_width + 1)
        self.conv_3 = convolution(in_channel, sentence_length - 1, filter_height, int(filter_channel / 3),
...
class ApLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        -------
        output : 2-D torch Tensor
            representation vector of size (batch_size, sentence_length)
        '''
